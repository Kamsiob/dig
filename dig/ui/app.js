...
function renderProject(){
  ...
  '<div class="ph"><div class="sq">'+esc(ini(p.name))+'</div><div><h1>'+esc(p.name)+'</h1><div class="m">'+tbadge(p)+'<span class="badge g">'+(p.pub?'Can be shared':'Private')+'</span><span class="badge g">'+HZ.find(function(h){return h[0]===(p.when||'later')})[1]+'</span>'+p.links.map(function(u){return '<a onclick="openLink('+jsq(u)+')">'+esc(u)+'</a>'}).join('')+'<a onclick="addLink(\''+p.id+'\')">+ link</a></div></div>'+
