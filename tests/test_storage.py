...
def test_create_and_read_idea(store: Store):
    idea = store.create_idea("Receipt filing that expires\nauto-deletes after 24h")
    assert idea is not None
    assert idea.title == "Receipt filing that expires"
...
def test_app_crud(store: Store):
    ...
    store.update_app(app.id, name="Model Station 2", shipped=False, notes="line\nline")
    refreshed = store.get_app(app.id)
    assert refreshed is not None
    assert refreshed.name == "Model Station 2"
...
def test_apps_are_listed_alphabetically(store: Store):
    for name in ["logbook", "a project in its last stage", "dig", "Model Station"]:
...
def test_promote_links_idea_to_app(store: Store):
    idea = store.create_idea("Notes for each video\none markdown file per video")
    assert idea is not None
    ...
    assert app.name == "Notes for each video"
