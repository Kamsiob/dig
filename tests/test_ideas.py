...
def test_promoted_ideas_are_hidden_then_shown_dimmed(window, store: Store):
    ...
    store.promote_idea(promoted.id, "Model Station")
    ...
    assert row.suffix.text() == "→ Model Station"
...
def test_opening_an_idea_loads_it(window, store: Store):
    idea = store.create_idea("Video notes\none file per video")
    assert idea is not None
    ...
    assert editor.title_field.text() == "Video notes"
...
def test_promote_prefills_the_app_editor(window, store: Store):
    ...
    assert editor.name_field.text() == "Video notes"
...
def test_an_app_needs_a_name_before_it_can_be_created(window, store: Store):
    ...
    editor.name_field.setText("a project in its last stage")
