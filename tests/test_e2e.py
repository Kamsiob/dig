...
def test_a_week_of_jotting_and_digging(profile, qtbot):
    ...
    for title, note in (
        ("Notes for each video", "one markdown file per video"),
        ("Receipt filing that expires", "everything goes after 24h"),
    ):
        field.setFocus()
...
def test_promoting_capturing_and_working_the_sheets(profile, qtbot):
    ...
    manual.name_field.setText("a project in its last stage")
    manual.github_field.setText("https://example.com/compass")
    ...
    bearings = by_name["a project in its last stage"]
...
def test_attachments_and_notes_survive(profile, qtbot, tmp_path, opened_files,
                                       no_confirm_needed):
    session = Session(profile, qtbot)
    app = session.store.create_app("Model Station")
...
def test_every_screen_holds_up_in_both_palettes(profile, qtbot, tmp_path):
    ...
    app = session.store.promote_idea(idea.id, "Model Station", "Does things")
