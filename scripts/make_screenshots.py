...
IDEAS = [
    ...
    ("Bulk image renamer", "rename by capture date and camera body", 31),
    ("Audio transcriber",
     "a local transcriber, drop a file in a folder", 20),
    ("Thumbnail tester",
     "track which thumbnail style correlates with CTR over time", 9),
    ("Feed digest",
     "daily channel post built from selected feeds, runs on a small server", 5),
    ("Receipt filing that expires",
...
def seed(store, workspace: Path):
    ...
    hub = store.promote_idea(
        origin.id,
        "Model Station",
        "Desktop control panel for the local model servers. Service "
        "control, model management, install-by-URL with hash verification, and "
        "a provenance manifest system.",
        github_url="https://example.com/model-station",
    ...
    for text in (
        ...
        "Provenance manifest via the download sources",
    ):
    ...
    for text in ("Window grouping wrong on Wayland", "Format warning fires twice"):
        item = store.add_sheet_item(hub.id, BUG, text)
        store.toggle_sheet_item(item.id)
    store.add_sheet_item(hub.id, BUG, "Status lamp stale after the model server restart")
    ...
    bearings = store.create_app(
        "a project in its last stage",
        "A living field guide for people switching to Bazzite Linux. "
        "Local-only, no telemetry.",
        github_url="https://example.com/compass",
    ...
    logbook = store.create_app(
        ...
        github_url="https://example.com/field-notes",
